...
class RealisasiKunjModel extends Model
{
    ...
    public function getDataBukaVerifikasiRealisasi($nik, $tanggal)
    {
        $builder = $this->db->table('trn_plan a')
            ->select("
                DISTINCT ON (a.cust_id) cust_id,
    ...
    public function getDataBukaVerifikasiRealisasiDet($nik, $tanggal, $pelanggan)
    {
        $builder = $this->db->table('trn_plan a')
            ->select("
                f_tv_user_cust_name(a.cust_user_id) AS cust_user_name,
    ...
    // Update data buka realisasi kunjungan berdasarkan ID
    public function updateBukaVerifRealisasi($cust_id, $nik, $date, $data)
    {
        // Update trn_real berdasarkan id_ref yang sesuai dengan trn_plan
        return $this->db->table('trn_real')
            ->whereIn('id_ref', function ($builder) use ($cust_id, $nik, $date) {
                $builder->select('id')
                    ->from('trn_plan')
                    ->where('cust_id', $cust_id)
                    ->where('nik', $nik)
